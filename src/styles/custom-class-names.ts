export const textbodyL = "text-[13px] leading-[23px] font-medium";
export const textBodyM = "text-xs leading-[15px] font-bold";
export const textHeadingXL = "text-[24px] font-bold";
export const textHeadingL = "text-[18px] leading-[23px] font-bold";
export const textHeadingM = "text-[15px] leading-[19px] font-bold";
export const textHeadingS =
  "text-[12px] leading-[15px] font-bold tracking-[2.4px]";
